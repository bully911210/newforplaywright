...
export async function getBrowserContext(workerId?: string): Promise<BrowserContext> {
  ...
  // Launch strategy per attempt:
  //   bundled Chromium → system Edge → system Chrome.
  // Bundled Chromium hits a Windows SxS bug on recent Windows 11 builds
  // (Activation context generation failed). Falling back to an installed
  // browser via Playwright channels avoids it entirely.
  const strategies: Array<{ label: string; opts: Record<string, unknown> }> = [
    { label: "bundled Chromium", opts: launchOptions },
    { label: "system Edge", opts: { ...launchOptions, channel: "msedge" } },
    { label: "system Chrome", opts: { ...launchOptions, channel: "chrome" } },
  ];

  for (let attempt = 1; attempt <= MAX_LAUNCH_RETRIES; attempt++) {
    try {
      const strategy = strategies[Math.min(attempt - 1, strategies.length - 1)];
      log("info", `[${key}] Launching ${strategy.label} (attempt ${attempt}/${MAX_LAUNCH_RETRIES}) at ${userDataDir}`);

      const ctx = await chromium.launchPersistentContext(userDataDir, strategy.opts as Parameters<typeof chromium.launchPersistentContext>[1]);
      ...
      log("info", `[${key}] Browser launched successfully via ${strategy.label}`);
      return ctx;
    } catch (err) {
